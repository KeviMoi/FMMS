...
<body>
                ...
                <div class="table">
                    ...
                    <div class="table_section">
                        <table id="schedulesTable">
                            ...
                            <tbody>
                                <?php
                                $query = "SELECT ms.schedule_id, ms.schedule_date, v.license_plate, ms.schedule_start_time, ms.schedule_end_time
                                ...
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    echo "<td>
                          <button onclick=\"admitVehicle({$row['schedule_id']}, '{$row['schedule_date']}')\"><i class='fa-solid fa-check'></i> Admit</button>
                        </td>";
                                    echo "</tr>";
                                }
                                ?>
    ...
    <script>
        ...
        function admitVehicle(scheduleId, scheduleDate) {
            const today = new Date().toISOString().split('T')[0];
            if (scheduleDate !== today) {
                Swal.fire({
                    icon: 'error',
                    title: 'Cannot Admit',
                    text: 'You can only admit vehicles scheduled for the current day.',
                });
                return;
            }
            ...
            Swal.fire({
                title: 'Admit Vehicle',
                text: 'Are you sure you want to admit this vehicle?',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    .then(data => {
                        Swal.fire({
                            icon: 'success',
                            title: 'Admitted',
                            text: 'The vehicle has been successfully admitted.',
                        }).then(() => {
                            location.reload(); // Reload the page to reflect the changes
                        });
                    })
                    .catch(error => {
                        console.error('Error admitting vehicle:', error);
                        Swal.fire({
                            icon: 'error',
                            title: 'Oops...',
                            text: 'Failed to admit the vehicle. Please try again.',
                        });
                    });
                }
            });
